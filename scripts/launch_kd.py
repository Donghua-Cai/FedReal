...
def kill_existing():
    patterns = [
        "python -m server.server_kd_main",
        "python -m client.client_kd_main",
    ]
    for pat in patterns:
        try:
...
def line_buffer_prefix():
    # 在 Linux/macOS 上优先用 stdbuf 让日志实时写文件；再配合 python -u
...
def parse_args():
    p = argparse.ArgumentParser(description="Launch KD-FL server and multiple clients")
    # Topology
    ...
    p.add_argument("--client_model", type=str, default="resnet18")
    ...
    p.add_argument("--log_dir", type=str, default="logs_kd")
